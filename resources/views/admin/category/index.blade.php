...
{{--    <div class="x-body">
      ...
      <xblock>
        ...
        {{--<button class="layui-btn" onclick="x_admin_show('添加用户','{{url('admin/category/create')}}')"><i class="layui-icon"></i>添加</button>--}}
          <a href="{{url('admin/category/create')}}" i class="layui-btn layui-icon">添加</a>
      ...
      <table class="layui-table">
        ...
        @foreach($data as $v)
          <tr>
            ...
            <td ><input type="text" style="width: 25px;" onchange="changeOrder(this,{{$v->cate_id}})" name="ord[]" value="{{$v->cate_order}}"></td>
            <td>{{$v->cate_id}}</td>
            <td>{{$v->_cate_name}}</td>
            <td>{{$v->cate_title}}</td>
            <td>{{$v->cate_view}}</td>
            <td class="td-manage">
             {{-- <a onclick="member_stop(this,'10001')" href="javascript:;"  title="启用">
              ...
              <a title="编辑" href="{{url('admin/category/'.$v->cate_id.'/edit ')}}">
                <i class="layui-icon">&#xe642;</i>
              </a>
              <a title="删除"  href="javascript:;" onclick="member_del({{$v->cate_id}});">
                <i class="layui-icon">&#xe640;</i>
              </a>
            </td>
          </tr>
    ...
    <script>
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
              //发异步删除数据
              $.post("{{url('admin/category/')}}/"+obj,{'_method':'delete','_token':"{{csrf_token()}}"},function(data){
                  if (data.status ==0){

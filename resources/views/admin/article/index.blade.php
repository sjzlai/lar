...
@section('content')
    <xblock>
        ...
        <a href="{{url('admin/article/create')}}" i class="layui-btn layui-icon">添加</a>
    ...
    <table class="layui-table">
        ...
        @foreach($data as $v)
            <tr>
                ...
                {{--</td>--}}
                <td>{{$v->art_title}}</td>
                <td><img src="{{$v->art_thumb}}" alt=""></td>
                <td>{{$v->art_tag}}</td>
                <td>{{$v->art_editor}}</td>
                <td>{{$v->art_view}}</td>
                <td>{{date('Y:m:d H:i:s',$v->art_time)}}</td>
                <td class="td-manage">
                    <a title="编辑" href="{{url('admin/category/'.$v->art_id.'/edit ')}}">
                        <i class="layui-icon">&#xe642;</i>
                    </a>
                    <a title="删除"  href="javascript:;" onclick="member_del({{$v->art_id}});">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
            </tr>
    ...
    <script>
        ...
        function member_del(obj,id){
            layer.confirm('确认要删除吗？',function(index){
                //发异步删除数据
                $.post("{{url('admin/article/')}}/"+obj,{'_method':'delete','_token':"{{csrf_token()}}"},function(data){
                    if (data.status ==0){
